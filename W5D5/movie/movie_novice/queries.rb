...
def top_titles
  ...
  good_movies = Movie.select(:id, :title).where('score >= ?', '9')
...
def star_wars
  ...
  # hint: use 'select' and 'where'
  star_warses = Movie.select(:id,:title,:yr).where('title LIKE ?', 'Star Wars%')
...
def below_average_years
  ...
  baddies = Movie.select('COUNT(*) AS bad_movies', :yr).where('score < ?', '5').group(:yr).order('yr DESC')
end
...
first_ten = Actor.select(:id, :name).order('name ASC').limit(10)
